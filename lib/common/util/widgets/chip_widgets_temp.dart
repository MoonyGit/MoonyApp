...
class SingleChoiceChipWidget extends ChipContainerWidget {
  ...
  const SingleChoiceChipWidget({
    required Map<dynamic, String> chipChoices,
    this.listener,
    this.chipSelected,
  }) : super(choices: chipChoices);

  /// Triggered when a chip is selected to update the selected chip
  final void Function(dynamic id, String selectedItem)? listener;
...
class _SingleChoiceChipState
  ...
  @override
  List<Widget> buildChipList() {
    return widget.choices.values.map((String item) {
      return ChoiceChip(
        label: SizedBox(
          width: 280,
          child: Text(
        ...
        onSelected: (bool selected) {
          ...
          widget.listener?.call(
            widget.choices.entries
                .firstWhere(
                  (MapEntry<dynamic, String> element) => element.value == item,
            )
                .key,
            item,
          );
...
class MultipleChoiceChipWidget extends ChipContainerWidget {
  ...
  final void Function(Map<dynamic, String> selectedItems)? listener;

  /// List of selected chips
  final Map<dynamic, String>? selectedItems;
...
class _MultipleChipState extends _ChipContainerState<MultipleChoiceChipWidget> {
  late final Map<dynamic, String> selectedItems =
      widget.selectedItems ?? Map<dynamic, String>.identity();
  ...
  List<Widget> buildChipList() {
    return widget.choices.values.map((String item) {
      return FilterChip(
        ...
        backgroundColor: AppTheme.background,
        selectedColor: AppTheme.primaryLight,
        selected: selectedItems.values.contains(item),
        onSelected: (bool value) {
          setState(() {
            if (value) {
              selectedItems.putIfAbsent(
                widget.choices.entries.firstWhere(
                      (MapEntry<dynamic, String> element) => element.value == item,
                ),
                    () => item,
              );
