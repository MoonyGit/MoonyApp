...
Future<Map<String, Material>> loadMtl(String fileName,
  ...
  final Map<String, Material> materials = Map<String, Material>();
  String data;
  try {
    if (isAsset) {
  ...
  for (String line in lines) {
    ...
    switch (parts[0]) {
      ...
      case 'Ka':
        if (parts.length >= 4) {
          final Vector3 v = Vector3(double.parse(parts[1]),
...
Future<Image> loadImageFromAsset(String fileName, {bool isAsset = true}) {
  ...
  dataFuture.then((Uint8List data) {
    instantiateImageCodec(data).then((Codec codec) {
...
Future<Uint32List> getImagePixels(Image image) async {
  final Completer<Uint32List> c = Completer<Uint32List>();
  image.toByteData(format: ImageByteFormat.rawRgba).then((ByteData? data) {
